...
describe('Game State Management', () => {
  ...
  test('preserves original hand indices after sorting the visible hand', () => {
    const hand = [
      { suit: 'S', rank: 'A' },
      { suit: 'E', rank: '10' },
      { suit: 'G', rank: 'O' },
    ...
    expect(sorted.map(({ originalIndex }) => originalIndex)).toEqual([2, 3, 1, 0]);
    expect(sorted[0].card).toEqual(hand[2]);
  });

  test('provides German suit labels for UI accessibility', () => {
    expect(getCardDisplayData({ suit: 'E', rank: 'O' }, GAME_TYPES.SAUSPIEL)).toEqual(
      expect.objectContaining({
        ...
        suitClass: 'eichel',
        isTrump: true,
        ariaLabel: 'Ober of Eichel, trump'
      })
    );
  });
});
